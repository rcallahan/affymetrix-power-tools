...
File5_debug_rec_t File5_debug_recs[32];
int File5_debug_recidx=0;

#define FILE5_DEBUG_INC_RECIDX(_idx)                      \
  {                                                       \
    _idx++;                                               \
    if (_idx>=NELMTS(File5_debug_recs)) {                 \
      _idx=0;                                             \
    }                                                     \
  }

void* File5_debug_malloc(size_t size)
{
  void* new_ptr=malloc(size);
  if (new_ptr!=NULL) {
    File5_debug_recs[File5_debug_recidx].m_ptr=new_ptr;
    File5_debug_recs[File5_debug_recidx].m_size=size;
    // advance
    FILE5_DEBUG_INC_RECIDX(File5_debug_recidx);
    // just have it print the stuff below for testing
    return new_ptr;
  }
  ...
  // Argh! the malloc failed...
  // print a notice about where it occured and return NULL.
  printf("File5_debug_malloc: failed attempting to malloc %d bytes\n",(int)size);
  int ii=File5_debug_recidx;
  for (int i=0;i<=NELMTS(File5_debug_recs);i++) {
    FILE5_DEBUG_INC_RECIDX(ii);
    if (File5_debug_recs[ii].m_ptr!=NULL) {
      printf("%2d : ptr=%p size=%10d\n",
             ii,
             File5_debug_recs[ii].m_ptr,
             File5_debug_recs[ii].m_size);
    }
  }
  printf("\n\n");
