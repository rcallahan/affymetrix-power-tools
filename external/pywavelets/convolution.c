...
// $Id: convolution.c.template 154 2010-03-13 13:18:59Z filipw $
                             ...
                             const_index_t step, MODE mode)
{
    ...
    if(F <= N){
        ...
        if(mode == MODE_PERIODIZATION){
            return double_downsampling_convolution_periodization(input, N, filter, F, output, step);
        ...
        return double_allocating_downsampling_convolution(input, N, filter, F, output, step, mode);
    }
}
                                        ...
                                        const_index_t step, MODE mode)
{
    ...
    if(mode != MODE_PERIODIZATION){
        ...
        buffer = (double*)wtcalloc(N_extended_len, sizeof(double));
        if(buffer == NULL)
            return -1;
                                    ...
                                    double* output, const_index_t O)
{
    ...
    // =======
    {
        ...
        periodization_buf = (double*)wtcalloc(N_p, sizeof(double));
        ...
        if(F_2%2){
            double_upsampling_convolution_valid_sf(periodization_buf, N_p, filter, F, output, O, MODE_ZEROPAD);
        ...
        // The F_2%2==0 case needs special result fix (oh my, another one..)
            ...
            double_upsampling_convolution_valid_sf(periodization_buf, N_p, filter, F, ptr_out, O, MODE_ZEROPAD);
    ...
    // Otherwise (N >= F_2)
        ...
        filter_even = (double*)wtmalloc(F_2 * sizeof(double));
        filter_odd = (double*)wtmalloc(F_2 * sizeof(double));
        ...
        // =======
            ...
            // whole input
            periodization_buf = (double*)wtcalloc(N_p, sizeof(double));
            periodization_buf_rear = (double*)wtcalloc(N_p, sizeof(double));
            ...
            if(k%2 == 1){
                ...
                *(ptr_out++) += sum_odd;
                ...
                --k;
                if(k)
                    double_upsampling_convolution_valid_sf(periodization_buf + 1, N_p-1, filter, F, ptr_out, O-1, MODE_ZEROPAD);
        ...
        // =======
            ...
            if(k%2 == 1){
                if(F/2 <= N_p - 1){ // k > 1 ?
                    double_upsampling_convolution_valid_sf(periodization_buf_rear , N_p-1, filter, F, ptr_out, O-1, MODE_ZEROPAD);
                }
                ...
                if(k){
                    double_upsampling_convolution_valid_sf(periodization_buf_rear, N_p, filter, F, ptr_out, O, MODE_ZEROPAD);
                }
            }
        }
        if(periodization_buf != NULL) wtfree(periodization_buf);
...
int double_upsampling_convolution_valid_sf(const double* input, const_index_t N,
    ...
    double sum_even, sum_odd;
    #ifdef OPT_UNROLL2
        double sum_even2, sum_odd2;
    #endif
    #ifdef OPT_UNROLL4
        #ifndef OPT_UNROLL2
            double sum_even2, sum_odd2;
        #endif
        double sum_even3, sum_odd3;
        double sum_even4, sum_odd4;
    #endif
    ...
    if(mode == MODE_PERIODIZATION) // Special case
        return double_upsampling_convolution_valid_sf_periodization(input, N, filter, F, output, O);
                             ...
                             const_index_t step, MODE mode)
{
    ...
    if(F <= N){
        ...
        if(mode == MODE_PERIODIZATION){
            return float_downsampling_convolution_periodization(input, N, filter, F, output, step);
        ...
        // reallocating memory for short signals (shorter than filter) is cheap
        return float_allocating_downsampling_convolution(input, N, filter, F, output, step, mode);
    }
}
                                        ...
                                        const_index_t step, MODE mode)
{
    ...
    if(mode != MODE_PERIODIZATION){
        ...
        buffer = (float*)wtcalloc(N_extended_len, sizeof(float));
        if(buffer == NULL)
            return -1;

        memcpy(buffer+F_minus_1, input, sizeof(float) * N);
        stop = N_extended_len;
    ...
    switch(mode){
        ...
        case MODE_ZEROPAD:
        default:
            //memset(buffer, 0, sizeof(float)*F_minus_1);
            //memset(buffer+N_extended_right_start, 0, sizeof(float)*F_minus_1);
            //memcpy(buffer+N_extended_right_start, buffer, sizeof(float)*F_minus_1);
            break;
    }
                                    ...
                                    float* output, const_index_t O)
{
    ...
    // =======
    {
        ...
        // periodization_buf will hold periodically copied input coeffs values
        periodization_buf = (float*)wtcalloc(N_p, sizeof(float));
        ...
        // Now perform the valid convolution
        if(F_2%2){
            float_upsampling_convolution_valid_sf(periodization_buf, N_p, filter, F, output, O, MODE_ZEROPAD);
	  ...
	  ptr_out = (float*)wtcalloc(idwt_buffer_length(N, F, MODE_PERIODIZATION), sizeof(float));
            if(ptr_out == NULL){
                wtfree(periodization_buf);
                return -1;
            }
            ...
            // Convolve here as for (F_2%2) branch above
            float_upsampling_convolution_valid_sf(periodization_buf, N_p, filter, F, ptr_out, O, MODE_ZEROPAD);
    ...
    // Otherwise (N >= F_2)
        ...
        // Allocate memory for even and odd elements of the filter
      filter_even = (float*)wtmalloc(F_2 * sizeof(float));
        filter_odd = (float*)wtmalloc(F_2 * sizeof(float));
        ...
        // =======
            ...
            // whole input
            periodization_buf = (float*)wtcalloc(N_p, sizeof(float));
            periodization_buf_rear = (float*)wtcalloc(N_p, sizeof(float));
            ...
            // Fill buffers with appriopriate elements
            memcpy(periodization_buf + N_p - k, input, k * sizeof(float));        // copy from beginning of input to end of buffer
            for(i = 1; i <= (N_p - k); ++i)                                        // kopiowanie 'cykliczne' od koñca input 
                periodization_buf[(N_p - k) - i] = input[N - (i%N)];

            memcpy(periodization_buf_rear, input + N - k, k * sizeof(float));    // copy from end of input to begginning of buffer
            for(i = 0; i < (N_p - k); ++i)                                        // kopiowanie 'cykliczne' od pocz¹tku input
                periodization_buf_rear[k + i] = input[i%N];
            ...
            if(k%2 == 1){
                ...
                *(ptr_out++) += sum_odd;
                ...
                --k;
                if(k)
                    float_upsampling_convolution_valid_sf(periodization_buf + 1, N_p-1, filter, F, ptr_out, O-1, MODE_ZEROPAD);
        ...
        // =======
            ...
            if(k%2 == 1){
                if(F/2 <= N_p - 1){ // k > 1 ?
                    float_upsampling_convolution_valid_sf(periodization_buf_rear , N_p-1, filter, F, ptr_out, O-1, MODE_ZEROPAD);
                }
                ...
                if(k){
                    float_upsampling_convolution_valid_sf(periodization_buf_rear, N_p, filter, F, ptr_out, O, MODE_ZEROPAD);
                }
            }
        }
        if(periodization_buf != NULL) wtfree(periodization_buf);
...
int float_upsampling_convolution_valid_sf(const float* input, const_index_t N,
    ...
    float sum_even, sum_odd;
    #ifdef OPT_UNROLL2
        float sum_even2, sum_odd2;
    #endif
    #ifdef OPT_UNROLL4
        #ifndef OPT_UNROLL2
            float sum_even2, sum_odd2;
        #endif
        float sum_even3, sum_odd3;
        float sum_even4, sum_odd4;
    #endif
    ...
    if(mode == MODE_PERIODIZATION) // Special case
        return float_upsampling_convolution_valid_sf_periodization(input, N, filter, F, output, O);
