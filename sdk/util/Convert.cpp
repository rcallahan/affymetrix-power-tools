...
void
Convert::strToDoubleVec(const std::string& s, const char delim, std::vector<double>& vec)
{
  ...
  for(int i = 0; i < words.size(); i++) {
    vec[i] = Convert::toDouble(words[i]);
  }
}
...
/**
 * Convert a double vector to string
 */
std::string Convert::doubleVecToString(const std::vector<double> &inputVector, const std::string &delim)
{
    if (inputVector.empty()) {
        return "";
    }
    ...
    std::string str;
    for (std::vector<double>::const_iterator it = inputVector.begin(); it != inputVector.end(); ++it)
    {
        str += delim + toString(*it);
    }
    return str.substr(delim.size());
