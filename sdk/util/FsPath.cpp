...
AptErr_t FsPath::setDirPath( const std::string & path ) {
  std::string temp1, temp2;
  return setPathHelper( path, temp1, temp2 );
}
...
AptErr_t FsPath::setPath( const std::string & path ) {
  ...
  AptErr_t aptErr = setPathHelper( path, tmp_os_path, tmp_internal_path );
...
AptErr_t FsPath::setPathHelper(const std::string& path,
                               std::string & tmp_os_path ,
                               std::string & tmp_internal_path ) {
...
bool FsPath::isAbsolute() const {
  ...
  std::string drive;
  Fs::splitPath(m_original_path, drive, dirs);
  if ((m_dir_name.find(FSPATH_PATHSEP)==0) || !drive.empty()) {
    return true;
  }
...
{
  Fs::chmodBasic(asUnixPath(),mode, false);
  return setErrFromFs();
}
...
AptErr_t FsPath::mkdir(bool errIfExists)
{
  if (dirExists()) {
    if (errIfExists) {
      setErr(APT_ERR, "FsPath::mkdir failed on path " + asString());
    }
    else {
      clearErr();
    }
...
AptErr_t FsPath::mkdirPath(bool errIfExists)
{
  ...
  FsPath tmp_path;
  tmp_path.setDirDrive(this->getDirDrive()); // needed for win only
  ...
  for (unsigned int i=0;i<dir_names.size();i++) {
    ...
    // only set errIfExists on last directory in path
    tmp_path.mkdir((i<dir_names.size()-1) ? false : errIfExists);
...
AptErr_t FsPath::rm(bool errIfNotExists)
{
  ...
  Fs::rm(asUnixPath(),m_err_doAbort);
  //return setErrFromFs();
  return APT_OK;
}
...
AptErr_t FsPath::listDir(std::vector<std::string>& names) {
  return Fs::listDir(asUnixPath(),names,m_err_doAbort);
}
...
int64_t FsPath::getFreeDiskSpace() const {
  return Fs::getFreeDiskSpace(asUnixPath(),m_err_doAbort);
}
