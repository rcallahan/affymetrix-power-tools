...
class Convert {
 ...
 public:
  ...
  /**
   * Utility function to cut a delimited string into a vector of doubles.
   *
   * @param s - delimited string.
   * @param delim - delimiter to chop on.
   * @param vec - vector to be filled in.
   */
  static void strToDoubleVec(const std::string& s, const char delim, std::vector<double>& vec);
  ...
  /**
   * The inverse of strToDoubleVec
   *
   * @param inputVector - the vector to convert
   * @param delim - delimiter to use
   * @return - the resulting string
   */
  static std::string doubleVecToString(const std::vector<double> &inputVector, const std::string &delim);
