...
public:
 ...
 int getArgCount(int oI = 0) {
  ...
  PgOpt* defineOption(const std::string& shortName,
    ...
    return m_Options[0].defineOption(shortName, longName, type, help, defaultVal, noHelp);
  }
  ...
  PgOpt* defOptMult(const std::string& shortName,
                    ...
                    const std::string& defaultVal,
                    const bool noHelp = false) {
    return m_Options[0].defOptMult(shortName, longName, type, help, defaultVal, noHelp);
  }
  ...
  void optionUsage(bool printOpts = true, int oI = 0) {
      m_Options[oI].usage(printOpts);
  }
