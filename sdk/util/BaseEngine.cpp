...
void BaseEngine::defineOptions() {
  ...
  defineOption("", "xml-file", PgOpt::STRING_OPT, 
                     ...
                     "");
  defineOption("", "xml-file-append-only", PgOpt::STRING_OPT, 
                     "Input parameters in XML format (Will not override command line settings).", 
                   "", true );
...
void BaseEngine::makeTempDir(std::string temp_dir) {
  ...
  if (Fs::dirExists(temp_dir)==false) {
    ...
    Fs::ensureWriteableDirPath(temp_dir, false);
  }
