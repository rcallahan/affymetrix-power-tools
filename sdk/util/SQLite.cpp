...
void SQLiteRecordset::open(const std::string& strSQL)
{
	const char* pszTail = NULL;
        ...
        for(int iIndex=0; iIndex < 10; iIndex++)
        {
            iReturnCode = sqlite3_prepare_v2(&(m_pdb->getConnection()), strSQL.c_str(), (int)strSQL.length(), &m_pstmt, &pszTail); 

            if (iReturnCode == SQLITE_BUSY)
            {
                Verbose::out(4, "SQLiteRecordset::open failed on attempt " + ::ToStr(iIndex) + " with sqlite3 error code " + ::ToStr(iReturnCode) );
                #ifndef WIN32 
                  struct timespec tWait, tRet;
                  tWait.tv_sec = 1;
                  tWait.tv_nsec = 0;
        ...
        if(iReturnCode != SQLITE_OK)
        {
            Verbose::out(4, "SQLiteRecordset::open failed for command \"" + strSQL + "\" with sqlite3 error code " + ::ToStr(iReturnCode) );
            m_pdb->error(iReturnCode, "Failed to prepare SQL statement.");  
        }
        m_bOpen = true;
        m_bFirstRow = true;
        m_bEOF = false;
