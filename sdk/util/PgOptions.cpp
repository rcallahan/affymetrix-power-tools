	...
	//						}
			...
			else
			{
				if (m_pPgOptions->isOptDefined(strOptionName))
				{
					...
					if ((strOptionName == "xml-file") || (strOptionName == "xml-file-append-only")) {
...
// @todo: this should be "bool PgOpt::isUserSet() {"
bool PgOpt::isSet() {
  // when the user supplies a value, is it stored in m_values with "setValue".
  // we can see if the user supplied a value by checking the size of the vector.
...
std::string PgOpt::getValue(size_t idx) const {
  ...
  // No value was set with setValue; return the default value.
  if (m_values.size()==0) {
    return getDefaultValue();
  }
  // return the user? value set from the command line.
...
PgOptions::PgOptions() : m_pXMLAppendOpts(NULL) {
  clear();
}
...
void PgOptions::clear() {
  ...
  if ( m_pXMLAppendOpts != NULL ) {
    delete m_pXMLAppendOpts;
  }
  m_pXMLAppendOpts = NULL;
...
PgOptions::PgOptions(const PgOptions &options) : m_pXMLAppendOpts(NULL) {
  m_progName = options.m_progName;
  m_argv = options.m_argv;
  m_args = options.m_args;
  m_usageMsg = options.m_usageMsg;
...
PgOpt* PgOptions::defOptMult(const std::string& shortName,
                             ...
                             const bool noHelp)
{
  PgOpt* opt=defineOption(shortName,longName,type,help,defaultValue, noHelp);
...
int PgOptions::parseArgv(const char * const * const argv, int start) {
  ...
  if ( m_pXMLAppendOpts != NULL ) {
    ...
    for (size_t i=0;i<m_option_vec.size();i++) {
      PgOpt *opt = m_option_vec[i];
      if ( ! opt->isSet() ) {
        PgOpt *optAppend = m_pXMLAppendOpts->mustFindOpt(opt->m_longName);
        if ( optAppend->isSet() ) {
          Verbose::out(1, opt->m_longName + " using XML file value...");
          m_option_vec[i] = new PgOpt(*optAppend);
          m_option_map[opt->m_longName] = m_option_vec[i];
          delete opt;
        }
      }
      else {
        Verbose::out(1,opt->m_longName + " using command line value...");
      }
    }
...
//  }
  ...
  if (opt->checkParseIsOk(arg_val)==1) {
    if (opt->m_allowMultiple==1) {
      ...
      if ((opt->m_longName == "xml-file") || (opt->m_longName == "xml-file-append-only")) {
                bool append_only = false;
		if (opt->isSet()) {Err::errAbort("An xml-file option has already been set. Only one xml-file option can be specified.");}
                if (opt->m_longName == "xml-file-append-only" ) {
                  PgOpt *xmlFileOpt = mustFindOpt("xml-file");
                  if (xmlFileOpt->isSet() ){Err::errAbort("An xml-file option has already been set. Only one xml-file option can be specified.");}
                  xmlFileOpt->setValue(arg_val);
                  append_only = true;
                }
		std::vector<std::string> vFileNames;
                if ( append_only ) {
                  Verbose::out(1, "Creating XML append options.");
                  m_pXMLAppendOpts = new PgOptions(*this);
                  m_pXMLAppendOpts->setOptionsFromXMLFile(arg_val, vFileNames);
                }
                else {
