...
static void define_birdseed_options(PgOptions* opts)
{
  ...
  opts->defineOption("", "priors-text", PgOpt::STRING_OPT,
                     "A text file containing priors (one per SNP).  This can also be used when writing a binary priors file to specify the priors to put in the binary file.  In that case all priors in the text file are included in the binary file.",
                     "");
  
  /// Writing prior files.
  opts->defineOption("", "write-priors-binary", PgOpt::STRING_OPT,
                     "Create a binary priors file.  If using this option, --priors-directory or --priors-text option must be used to specify input priors.",
                     "");
  opts->defineOption("", "write-priors-tsv", PgOpt::STRING_OPT,
                     "Create a tsv priors file.  If using this option, one of the --priors- options must be used to specify input priors.",
                     "");
  opts->defineOption("", "write-priors-text", PgOpt::STRING_OPT,
                     "Create a text priors file.  If using this option, one of the --priors- options must be used to specify input priors.",
                     "");
  
  opts->defineOption("", "clusters", PgOpt::STRING_OPT,
                     "Instead of calculating clusters, read clusters for each SNP from the given file, and use these to call genotypes.  This option is mutually exclusive with --priors-text and --priors-binary",
                     "");
  
  opts->defineOption("", "chrX-snps", PgOpt::STRING_OPT,
                     "(deprecated) File containing snps on chrX (non-pseudoautosomal region). This file must have a header line \"all_chrx_no_par\".  This opt and --special-snps are mutually exclusive.  --special-snps is preferred to this option.  For a chip with mitochondrial SNPs, --special-snps should be used.",
                     "");
  
  opts->defineOption("", "special-snps", PgOpt::STRING_OPT,
                     "File containing snps of unusual copy number (X, Y, mito).  This opt and --chrX-snps are mutually exclusive.",
                     "");
  
  opts->defineOption("", "gender-file", PgOpt::STRING_OPT,
                     "File containing a line for each sample in the data file.  0=female, 1=male, 2=unknown. This file must have a header line \"gender\".",
                     "");
  ...
  opts->defineOption("c", "correction-factor", PgOpt::DOUBLE_OPT,
                     "Use the supplied value rather than the regular mechanism for calculating the prior correction factor.  This is useful in debugging.",
                     "-1.0");
  ...
  opts->defineOption("v", "verbose", PgOpt::INT_OPT,
                     "How verbose to be with status messages 0 - quiet, 1 - write clusters to stdout, 2 - more messages, 3 - even more messages.",
                     "0");
...
BasicPriorsReader* open_priorsSource (PgOptions& opts,std::string* path)
{
  ...
  p=opts.get("priors-binary");
  if (p!="") {
    *path=p;
    return new BinaryPriorsReader(p);
  }
  p=opts.get("priors-tsv");
  if (p!="") {
    *path=p;
    return new TsvPriorsReader(p);
  }
  p=opts.get("priors-text");
  if (p!="") {
    *path=p;
    return new TextPriorsReader(p);
  }
  //
...
//      ++numOpts;
//    }
//    for (PgOpt **p = optionsDef; *p != NULL; ++p) {
//      ++numOpts;
//    }
//    combinedOpts = new PgOpt *[numOpts + 1];
//    pCombinedOpts = combinedOpts;
//    for (PgOpt **p = algorithmicOpts; *p != NULL; ++p) {
//      *pCombinedOpts++ = *p;
//    }
//    for (PgOpt **p = optionsDef; *p != NULL; ++p) {
...
//    *pCombinedOpts = NULL;
    ...
    if ((opts.get("write-priors-binary")!="") ||
      ...
      fprintf(stderr, "ERROR: When using converting files the '%s', '%s' or '%s' option must also be used.\n",
              opts.mustFindOpt("priors-binary")->m_longName.c_str(),
              opts.mustFindOpt("priors-tsv")->m_longName.c_str(),
              opts.mustFindOpt("priors-text")->m_longName.c_str());
      opts.usage();
      return 1;
    }
    ...
    bool calculateSNPSpecificCorrectionFactor = opts.getBool("snp-specific-correction-factor") && !opts.getBool("average-correction-factor");
    ...
    while (intensitiesParser.advanceSNP()) {
      ...
      for (size_t i = 0; i < intensitiesParser.getCurrentIntensities().numRows(); ++i) {
        ...
        // Convert NOCALL to -1
        // @todo: use something symbolic, not '-1'
