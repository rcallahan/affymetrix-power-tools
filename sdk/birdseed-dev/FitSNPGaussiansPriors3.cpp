...
/// @file FitSNPGaussiansPriors3.cpp
...
static double anchor_weight;
static double max_anchor_percentage;
// Not static because used by GenotypeCaller
double birdseed::dev::final_weight_min;
...
double birdseed::dev::relative_distance_confidence_weight;
...
static OptionSpec<double> doubleOptions[] = {
	...
	DEFINE_DOUBLE_OPTION(covar_floor_decay,8,"Covariance decays over this iteration scale.", true),
    ...
    DEFINE_DOUBLE_OPTION(anchor_weight, 1.0, "how strongly to anchor clusters to the priors (using pseudocounts).", true),
    DEFINE_DOUBLE_OPTION(max_anchor_percentage, 5.0, "maximum strength of anchor to priors (as a percentage of samples in the cluster).", true),
    ...
    DEFINE_DOUBLE_OPTION(mono_het_penalty, 999999999, "Multiply the distance squared between cluster and prior in 1-cluster model by this, when trying to fit single cluster to AB prior.", true),
    DEFINE_DOUBLE_OPTION(two_cluster_low_observation_penalty_factor, 10, "When penalizing an alignment of two-cluster model, use this factor to place a floor on penalty when number of prior observations for a prior is low.", true),
    ...
    DEFINE_DOUBLE_OPTION(mean_dist2, 1.2, "if two means get this close, penalize ll by 3k*log(n)/2;.", true),
    DEFINE_DOUBLE_OPTION(lambda3, 2.5, "how much to penalize differences from mean_dist2.", true)
};
...
PgOpt **birdseed::dev::getAlgorithmicOpts()
{
    return pgOptions;
}

void birdseed::dev::setAlgorithmicParameters(CONSTHACK PgOptions &opts)
{
    for (size_t i = 0; i < ARRAY_LEN(doubleOptions); ++i) {
...
void birdseed::dev::getSelfDocOptions(vector<SelfDoc::Opt> *opts)
{
    for (size_t i = 0; i < ARRAY_LEN(doubleOptions); ++i) {
...
void birdseed::dev::setSelfDocOptions(SelfDoc *doc, CONSTHACK std::map<std::string,std::string> &param)
{
    for (size_t i = 0; i < ARRAY_LEN(doubleOptions); ++i) {
        if (!doubleOptions[i].selfDocOpt.name.empty()) {
...
void birdseed::dev::setFitSNPGaussiansVerbosity(int verbose)
{
    verbosity = verbose;
}
                                 ...
                                 const FixedVector<double, NUM_ALLELES> &stdInterceptPrior)
{
	...
	double expMeans[3][2];
	if(bestpriormatch==0){
	  expMeans[0][0] = means[0];
	  expMeans[0][1] = means[1];
	  expMeans[1][0] = means[0]*0.634 + means[1]*0.180;
	  expMeans[1][1] = means[0]*0.389 + means[1]*1.011;
	  expMeans[2][0] = means[0]*0.140 + means[1]*0.506;
	  expMeans[2][1] = means[0]*0.679 + means[1]*1.077;
	} else if(bestpriormatch==2){
	  expMeans[0][0] = means[0]*1.008 + means[1]*0.684;
	  expMeans[0][1] = means[0]*0.470 + means[1]*0.131;	  
	  expMeans[1][0] = means[0]*0.972 + means[1]*0.392;
	  expMeans[1][1] = means[0]*0.167 + means[1]*0.630;
	  expMeans[2][0] = means[0];
	  expMeans[2][1] = means[1];
	} else { //bestpriormatch is the het cluster
	  expMeans[0][0] = means[0]*1.489 + means[1]*(0-0.027);
	  expMeans[0][1] = means[0]*0.075 + means[1]*0.316;	  
	  expMeans[1][0] = means[0];
	  expMeans[1][1] = means[1];
	  expMeans[2][0] = means[0]*0.310 + means[1]*0.098;
	  expMeans[2][1] = means[0]*(0-0.010) + means[1]*1.483;
	}
	
	ret->means.resize(priors.getNumPriors());
    for (size_t i = 0; i < priors.getNumPriors(); ++i) {
	  if (i==bestpriormatch){
		ret->means[i][0] = ((expMeans[i][0] * numSamples) + (priors.getPrior(i).m_mean[0] * anchor_weight)) / (numSamples + anchor_weight);
		ret->means[i][1] = ((expMeans[i][1] * numSamples) + (priors.getPrior(i).m_mean[1] * anchor_weight)) / (numSamples + anchor_weight);
	  } else {
		ret->means[i][0] = (expMeans[i][0] + (priors.getPrior(i).m_mean[0]* max_anchor_percentage)) / (1+max_anchor_percentage);
		ret->means[i][1] = (expMeans[i][1] + (priors.getPrior(i).m_mean[1]* max_anchor_percentage)) / (1+max_anchor_percentage);
	  }
    }
...
static void fillInMissingGaussians(Clusters *clusters, const Priors &priors, size_t numSamples)
{
	...
	double expMeans[3][2];
	if((bestpriormatch[0]==0) && (bestpriormatch[1]==1)){
	  expMeans[0][0] = means[0][0];
	  expMeans[0][1] = means[0][1];
	  expMeans[1][0] = means[1][0];
	  expMeans[1][1] = means[1][1];
	  expMeans[2][0] = means[0][0]*(0-1.048) + means[0][1]*0.149 + means[1][0]*1.860 + means[1][1]*0.023;
	  expMeans[2][1] = means[0][0]*0.123 + means[0][1]*(0-0.586) + means[1][0]*(0-0.149) + means[1][1]*1.672;
	} else if((bestpriormatch[0]==1) && (bestpriormatch[1]==2)){
	  expMeans[0][0] = means[0][0]*1.675 + means[0][1]*(0-0.147) + means[1][0]*(0-.596) + means[1][1]*0.120;
	  expMeans[0][1] = means[0][0]*0.020 + means[0][1]*1.831 + means[1][0]*0.143 + means[1][1]*(0-1.031);
	  expMeans[1][0] = means[0][0];
	  expMeans[1][1] = means[0][1];
	  expMeans[2][0] = means[1][0];
	  expMeans[2][1] = means[1][1];
	} else { //bestpriormatch is the hom/hom
	  expMeans[0][0] = means[0][0];
	  expMeans[0][1] = means[0][1];
	  expMeans[1][0] = means[0][0]*0.585 + means[0][1]*(0-0.008) + means[1][0]*0.387 + means[1][1]*(0-0.007);
	  expMeans[1][1] = means[0][0]*(0-0.008) + means[0][1]*0.382 + means[1][0]*(0-0.003) + means[1][1]*0.586;
	  expMeans[2][0] = means[1][0];
	  expMeans[2][1] = means[1][1];
	}
	
    for (size_t i = 0; i < priors.getNumPriors(); ++i) {
	  if (i==bestpriormatch[0]){
		clusters->means[i][0] = ((expMeans[i][0] * oldWeights[0] * numSamples) + (priors.getPrior(i).m_mean[0] * anchor_weight)) / (oldWeights[0] * numSamples + anchor_weight);
		clusters->means[i][1] = ((expMeans[i][1] * oldWeights[0] * numSamples) + (priors.getPrior(i).m_mean[1] * anchor_weight)) / (oldWeights[0] * numSamples + anchor_weight);
	  } else if (i==bestpriormatch[1]){
		clusters->means[i][0] = ((expMeans[i][0] * oldWeights[1] * numSamples) + (priors.getPrior(i).m_mean[0] * anchor_weight)) / (oldWeights[1] * numSamples + anchor_weight);
		clusters->means[i][1] = ((expMeans[i][1] * oldWeights[1] * numSamples) + (priors.getPrior(i).m_mean[1] * anchor_weight)) / (oldWeights[1] * numSamples + anchor_weight);
	  } else {
		clusters->means[i][0] = (expMeans[i][0] + (priors.getPrior(i).m_mean[0]* max_anchor_percentage)) / (1+max_anchor_percentage);
		clusters->means[i][1] = (expMeans[i][1] + (priors.getPrior(i).m_mean[1]* max_anchor_percentage)) / (1+max_anchor_percentage);
	  }
    }
	...
	for (size_t i = 0; i < MAX_NUM_CLUSTERS; ++i) {
	  if(i==bestpriormatch[0]) {
        clusters->means[i][0] = (numSamples * oldWeights[0] * (bestshift1 + priors.getPrior(i).m_mean[0] * bestscale1) + (anchor_weight * priors.getPrior(i).m_mean[0]))/(numSamples * oldWeights[0] + anchor_weight);
        clusters->means[i][1] = (numSamples * oldWeights[0] * (bestshift2 + priors.getPrior(i).m_mean[1] * bestscale2) + (anchor_weight * priors.getPrior(i).m_mean[1]))/(numSamples * oldWeights[0] + anchor_weight);
	  } else if(i==bestpriormatch[1]) {
        clusters->means[i][0] = (numSamples * oldWeights[1] * (bestshift1 + priors.getPrior(i).m_mean[0] * bestscale1) + (anchor_weight * priors.getPrior(i).m_mean[0]))/(numSamples * oldWeights[1] + anchor_weight);
        clusters->means[i][1] = (numSamples * oldWeights[1] * (bestshift2 + priors.getPrior(i).m_mean[1] * bestscale2) + (anchor_weight * priors.getPrior(i).m_mean[1]))/(numSamples * oldWeights[1] + anchor_weight);
	  } else {
        clusters->means[i][0] = (bestshift1 + priors.getPrior(i).m_mean[0] * (bestscale1 + max_anchor_percentage))/(1+max_anchor_percentage);
        clusters->means[i][1] = (bestshift2 + priors.getPrior(i).m_mean[1] * (bestscale2 + max_anchor_percentage))/(1+max_anchor_percentage);
	  }
    }
	*/
...
void birdseed::dev::calculatePXI_ZJ(PXI_ZJMatrix *pxi_zj,
                         ...
                         size_t iter)
{
    ...
    for (size_t j = 0; j < k; ++j) {
        //Now With Anchoring:
	    double aweightj = min(nj[j]*max_anchor_percentage, aweight);

        //means(j,:) = (min(nj(j)/3,anchorstrength)*priors(j,1:2)+sum(repmat(pxi_zj(:,j),1,2).*dataVector)) / (min(nj(j)/3,anchorstrength)+nj(j));
        for (size_t col = 0; col < NUM_ALLELES; ++col) {
            double meanSum = 0;
            for (size_t row = 0; row < intensities.numRows(); ++row) {
                meanSum += clusters->pxi_zj[row][j] * intensities[row][col];
            }
			meanSum += aweightj * priors.getPrior(j).m_mean[col];
            means[j][col] = meanSum / (aweightj + nj[j]);
        }
        ...
        for (size_t col = 0; col < NUM_ALLELES; ++col) {
            ...
            double priormean = priors.getPrior(j).m_mean[col];
			varSum += aweightj * ((priormean - means[j][col]) * (priormean - means[j][col]));
            clusters->vars[j][col] = varSum/(aweightj + nj[j]);
        }
		...
		covSum += aweightj *
		    (priors.getPrior(j).m_mean[0] - means[j][0]) *
  		    (priors.getPrior(j).m_mean[1] - means[j][1]);
        covs[j] = covSum / (aweight + nj[j]);
    }
    ...
    for (size_t d = 0; d < 2; ++d) {
        ...
        expectedvars = powVector(clusters->means.columnSlice(d)*std_slope +
                                 max(30.0,meanVector(sqrtVector(clusters->vars.columnSlice(d)) -
                                                     clusters->means.columnSlice(d)*std_slope)), 2);
...
Clusters birdseed::dev::FitSNPGaussianPriors3(const IntensityMatrix &intensities, const Priors &priors)
{
...
double birdseed::dev::calculateSNPSpecificCorrectionFactor(const IntensityMatrix &intensities,
                                                      const Priors &priors)
{
	...
	double numobstot = 0.0;
	double pseudos = 0.1; //Make sure >0 so don't get divide by 0 error
    for (size_t i = 0; i < priors.getNumPriors(); ++i) {
	    //old: denominator += sqrt(pow(priors.getPrior(i).m_mean[0], 2) + pow(priors.getPrior(i).m_mean[1], 2));
	    //Each prior should be weighted by the number of observations for that prior (and then divide by the total number of observations); made more important due to anchoring
        double numobscur = priors.getPrior(i).m_numObservations + pseudos;
	    numobstot += numobscur;
        double mean0 = priors.getPrior(i).m_mean[0];
        double mean1 = priors.getPrior(i).m_mean[1];
        denominator += sqrt((mean0 * mean0) + (mean1 * mean1)) * numobscur;
    }
    //old: denominator /= priors.getNumPriors();
	denominator /= numobstot;
    return numerator / denominator;
}

double birdseed::dev::calculateConfidenceRelativeToCluster(const Matrix2x2 &B,
...
Matrix2x2 birdseed::dev::makeBMatrix(const Matrix2x2::ROW &clusterVars, double covar)
{
    Matrix2x2 varmat;
