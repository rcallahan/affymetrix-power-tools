...
void CHPQuantificationDetectionFileBufferWriter::Initialize(std::vector<std::string> *CHPFileNames)
{
	...
	{
		std::vector<std::pair<float, float> > quantificationBuffer;
...
void CHPQuantificationDetectionFileBufferWriter::WriteQuantificationDetectionEntry(int target, float quantification, float pvalue)
{
	m_TargetQuantificationDetectionBuffers[target].push_back(std::pair<float, float>(quantification, pvalue));
	m_BufferSize += sizeof(std::pair<float, float>);
...
void CHPQuantificationDetectionFileBufferWriter::FlushBuffer()
{
	if(m_BufferSize > 0) {
		for (int target=0; target<m_CHPFileNames->size(); target++)
		{
			...
			updater.UpdateExpressionQuantificationDetectionBuffer(m_TargetQuantificationDetectionRowIndexes[target], m_TargetQuantificationDetectionBuffers[target]);
