...
void CalvinCHPFileUpdater::UpdateExpressionQuantificationDetectionBuffer(int row_start, std::vector<std::pair<float,float> > &expressionSignalBuffer)
{
	// seek to start of update row (note NAME_COLUMN is 0)
	SeekToPosition(*m_CHPFile, ENTRY_DATA_GROUP, ENTRY_DATA_SET, row_start, NAME_COLUMN);
	int iProbeSetNameColumnSize = colsizes[ENTRY_DATA_GROUP][ENTRY_DATA_SET][NAME_COLUMN];
	...
	{
		m_CHPFile->seekp(iProbeSetNameColumnSize, std::ios::cur);
		FileOutput::WriteFloat(*m_CHPFile, expressionSignalBuffer[i].first);
		FileOutput::WriteFloat(*m_CHPFile, expressionSignalBuffer[i].second);
	}
}
