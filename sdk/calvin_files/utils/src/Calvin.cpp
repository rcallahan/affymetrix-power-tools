...
//							pDataSet2->Delete();
						...
						if (!bEquals)
						{
							...
							{
								iFailureCount++;
								float fAbsDiff = fabs(f1 - f2);
								if (fAbsDiff > vMaxDifference.get(uiColIndex) || (flagNaNNumDiff && fAbsDiff != fAbsDiff))
								{
									vMaxDifference.set(uiColIndex, fAbsDiff);
					...
					if (vMaxDifference.get(uiColIndex) > 0 || vMaxDifference.get(uiColIndex) != vMaxDifference.get(uiColIndex))
					{
