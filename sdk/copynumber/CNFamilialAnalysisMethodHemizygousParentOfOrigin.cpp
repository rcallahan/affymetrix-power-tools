...
/**
 * @file CNFamilialAnalysisMethodHemizygousParentOfOrigin.cpp
 *
 * @brief This file contains the CNFamilialAnalysisMethodHemizygousParentOfOrigin class members.
 */
 ...
 * @return SelfDoc
 */
SelfDoc CNFamilialAnalysisMethodHemizygousParentOfOrigin::explainSelf()
{
    CNFamilialAnalysisMethodHemizygousParentOfOrigin obj;
    SelfDoc doc;
 ...
 */
std::vector<SelfDoc::Opt> CNFamilialAnalysisMethodHemizygousParentOfOrigin::getDefaultDocOptions()
{
  std::vector<SelfDoc::Opt> opts;

  // SelfDoc::Opt(name, type, value, default, min, max, description)

  SelfDoc::Opt opt1 = {getType() + "-marker-count-cutoff", SelfDoc::Opt::Integer, "21", "21", "1", "NA", "HemizygousParentOfOrigin Marker Count Cutoff"};
  opts.push_back(opt1);

  SelfDoc::Opt opt2 = {getType() + "-call-cutoff", SelfDoc::Opt::Integer, "4", "4", "0", "NA",  "HemizygousParentOfOrigin Call Cutoff"};
  opts.push_back(opt2);

  SelfDoc::Opt opt3 = {getType() + "-min-genomic-span", SelfDoc::Opt::Integer, "1000000", "1000000", "1", "NA", "HemizygousParentOfOrigin Minimum Genomic Span"};
  opts.push_back(opt3);

  SelfDoc::Opt opt4 = {getType() + "-xChromosome", SelfDoc::Opt::Integer, "24", "24", "1", "NA", "HemizygousParentOfOrigin X Chromosome"};
  opts.push_back(opt4);

  SelfDoc::Opt opt5 = {getType() + "-yChromosome", SelfDoc::Opt::Integer, "25", "25", "1", "NA", "HemizygousParentOfOrigin Y Chromosome"};
  opts.push_back(opt5);
 ...
 */
SelfCreate* CNFamilialAnalysisMethodHemizygousParentOfOrigin::newObject(std::map<std::string,std::string>& params)
{
    SelfDoc doc = explainSelf();
    std::vector<SelfDoc::Opt> opts = getDefaultDocOptions();
    CNFamilialAnalysisMethodHemizygousParentOfOrigin* pMethod = new CNFamilialAnalysisMethodHemizygousParentOfOrigin();
 ...
 * @brief Constructor
 */
CNFamilialAnalysisMethodHemizygousParentOfOrigin::CNFamilialAnalysisMethodHemizygousParentOfOrigin()
{
 ...
 * Hemizygous is when the index inherets only one allele from only parent.
 * (CN=1 + genotype concordance with one parent)
 */
void CNFamilialAnalysisMethodHemizygousParentOfOrigin::run()
{
    Verbose::out(1, "CNFamilialAnalysisMethodHemizygousParentOfOrigin::run(...) start");
    ...
    {
        Verbose::progressBegin(1, "CNFamilialAnalysisMethodHemizygousParentOfOrigin::run(...) Mother's segments ", iProbeSetCount, 100000, (iProbeSetCount / 100000));
...
//                    if (pIndex->getCNLoss(cychpIndex.getCychpHeader().getGender(), m_iXChromosome, m_iYChromosome) == 1)
    ...
    if (cychpFather.getFamilialCall() == 1)
    {
        Verbose::progressBegin(1, "CNFamilialAnalysisMethodHemizygousParentOfOrigin::run(...) Father's segments ", iProbeSetCount, 100000, (iProbeSetCount / 100000));
        for (int i = 0; (i < iProbeSetCount); i++)
        {
            Verbose::progressStep(1);
            CNCychpProbeSetsCopyNumber* pIndex = cychpIndex.getCychpProbeSetsCopyNumbers().getAt(i);
...
//                    if (pIndex->getCNLoss(cychpIndex.getCychpHeader().getGender(), m_iXChromosome, m_iYChromosome) == 1)
    ...
    Verbose::out(1, "CNFamilialAnalysisMethodHemizygousParentOfOrigin::run(...) end");
}
