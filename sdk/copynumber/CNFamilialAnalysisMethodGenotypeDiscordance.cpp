...
/**
 * @file CNFamilialAnalysisMethodGenotypeDiscordance.cpp
 *
 * @brief This file contains the CNFamilialAnalysisMethodGenotypeDiscordance class members.
 */
 ...
 * @return SelfDoc
 */
SelfDoc CNFamilialAnalysisMethodGenotypeDiscordance::explainSelf()
{
    CNFamilialAnalysisMethodGenotypeDiscordance obj;
    SelfDoc doc;
 ...
 */
std::vector<SelfDoc::Opt> CNFamilialAnalysisMethodGenotypeDiscordance::getDefaultDocOptions()
{
  std::vector<SelfDoc::Opt> opts;

  // SelfDoc::Opt(name, type, value, default, min, max, description)

  SelfDoc::Opt opt1 = {getType() + "-marker-count-cutoff", SelfDoc::Opt::Integer, "21", "21", "1", "NA", "GenotypeDiscordance Marker Count Cutoff"};
  opts.push_back(opt1);

  SelfDoc::Opt opt2 = {getType() + "-call-cutoff", SelfDoc::Opt::Integer, "4", "4", "0", "NA", "GenotypeDiscordance Call Cutoff"};
  opts.push_back(opt2);

  SelfDoc::Opt opt3 = {getType() + "-min-genomic-span", SelfDoc::Opt::Integer, "1000000", "1000000", "1", "NA", "GenotypeDiscordance Minimum Genomic Span"};
  opts.push_back(opt3);
 ...
 */
SelfCreate* CNFamilialAnalysisMethodGenotypeDiscordance::newObject(std::map<std::string,std::string>& params)
{
    SelfDoc doc = explainSelf();
    std::vector<SelfDoc::Opt> opts = getDefaultDocOptions();
    CNFamilialAnalysisMethodGenotypeDiscordance* pMethod = new CNFamilialAnalysisMethodGenotypeDiscordance();
 ...
 * @brief Constructor
 */
CNFamilialAnalysisMethodGenotypeDiscordance::CNFamilialAnalysisMethodGenotypeDiscordance()
{
 ...
 */
void CNFamilialAnalysisMethodGenotypeDiscordance::run()
{
    Verbose::out(1, "CNFamilialAnalysisMethodGenotypeDiscordance::run(...) start");
    ...
    {
        Verbose::progressBegin(1, "CNFamilialAnalysisMethodGenotypeDiscordance::run(...) Mother's segments ", iProbeSetCount, 100000, (iProbeSetCount / 100000));
        for (int i = 0; (i < iProbeSetCount); i++)
        {
            ...
            {
                pMother->setSegmentInput((isGenotypeConcordance(pIndex, pMother)) ? (char)1 : (char)0);
            }
        }
        anneal(cychpMother);
    ...
    {
        Verbose::progressBegin(1, "CNFamilialAnalysisMethodGenotypeDiscordance::run(...) Father's segments ", iProbeSetCount, 100000, (iProbeSetCount / 100000));
        for (int i = 0; (i < iProbeSetCount); i++)
        {
            Verbose::progressStep(1);
            CNCychpProbeSetsCopyNumber* pIndex = cychpIndex.getCychpProbeSetsCopyNumbers().getAt(i);
            ...
            {
                pFather->setSegmentInput((isGenotypeConcordance(pIndex, pFather)) ? (char)1 : (char)0);
            }
        }
        anneal(cychpFather);
    ...
    Verbose::out(1, "CNFamilialAnalysisMethodGenotypeDiscordance::run(...) end");
}
