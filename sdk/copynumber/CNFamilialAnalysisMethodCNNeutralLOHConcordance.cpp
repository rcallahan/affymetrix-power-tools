...
/**
 * @file CNFamilialAnalysisMethodCNNeutralLOHConcordance.cpp
 *
 * @brief This file contains the CNFamilialAnalysisMethodCNNeutralLOHConcordance class members.
 */
 ...
 * @return SelfDoc
 */
SelfDoc CNFamilialAnalysisMethodCNNeutralLOHConcordance::explainSelf()
{
    CNFamilialAnalysisMethodCNNeutralLOHConcordance obj;
    SelfDoc doc;
 ...
 */
std::vector<SelfDoc::Opt> CNFamilialAnalysisMethodCNNeutralLOHConcordance::getDefaultDocOptions()
{
  std::vector<SelfDoc::Opt> opts;

  // SelfDoc::Opt(name, type, value, default, min, max, description)

  SelfDoc::Opt opt1 = {getType() + "-marker-count-cutoff", SelfDoc::Opt::Integer, "21", "21", "1", "NA", "CNNeutralLOHConcordance Marker Count Cutoff"};
  opts.push_back(opt1);

  SelfDoc::Opt opt2 = {getType() + "-call-cutoff", SelfDoc::Opt::Integer, "4", "4", "0", "NA",  "CNNeutralLOHConcordance Call Cutoff"};
  opts.push_back(opt2);

  SelfDoc::Opt opt3 = {getType() + "-min-genomic-span", SelfDoc::Opt::Integer, "1000000", "1000000", "1", "NA", "CNNeutralLOHConcordance Minimum Genomic Span"};
  opts.push_back(opt3);

  SelfDoc::Opt opt4 = {getType() + "-xChromosome", SelfDoc::Opt::Integer, "24", "24", "1", "NA", "CNNeutralLOHConcordance X Chromosome"};
  opts.push_back(opt4);

  SelfDoc::Opt opt5 = {getType() + "-yChromosome", SelfDoc::Opt::Integer, "25", "25", "1", "NA", "CNNeutralLOHConcordance Y Chromosome"};
  opts.push_back(opt5);
 ...
 */
SelfCreate* CNFamilialAnalysisMethodCNNeutralLOHConcordance::newObject(std::map<std::string,std::string>& params)
{
    SelfDoc doc = explainSelf();
    std::vector<SelfDoc::Opt> opts = getDefaultDocOptions();
    CNFamilialAnalysisMethodCNNeutralLOHConcordance* pMethod = new CNFamilialAnalysisMethodCNNeutralLOHConcordance();
 ...
 * @brief Constructor
 */
CNFamilialAnalysisMethodCNNeutralLOHConcordance::CNFamilialAnalysisMethodCNNeutralLOHConcordance()
{
 ...
 */
void CNFamilialAnalysisMethodCNNeutralLOHConcordance::run()
{
    Verbose::out(1, "CNFamilialAnalysisMethodCNNeutralLOHConcordance::run(...) start");
    ...
    {
        Verbose::progressBegin(1, "CNFamilialAnalysisMethodCNNeutralLOHConcordance::run(...) Mother's segments ", iProbeSetCount, 100000, (iProbeSetCount / 100000));
        for (int i = 0; (i < iProbeSetCount); i++)
        {
            ...
            else
            {
                ...
                {
                    if (pIndex->getCNNeutral(cychpIndex.getCychpHeader().getGender(), m_iXChromosome, m_iYChromosome) == 1)
                    {
    ...
    {
        Verbose::progressBegin(1, "CNFamilialAnalysisMethodCNNeutralLOHConcordance::run(...) Father's segments ", iProbeSetCount, 100000, (iProbeSetCount / 100000));
        for (int i = 0; (i < iProbeSetCount); i++)
        {
            Verbose::progressStep(1);
            CNCychpProbeSetsCopyNumber* pIndex = cychpIndex.getCychpProbeSetsCopyNumbers().getAt(i);
    ...
    Verbose::out(1, "CNFamilialAnalysisMethodCNNeutralLOHConcordance::run(...) end");
}
