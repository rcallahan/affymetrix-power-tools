...
int main(int argc,const char* argv[]) {
  ...
  try {
    ProbesetGenotypeStageEngine engine;
    
    const string version ("NON-OFFICIAL-RELEASE");
    const string cvsId ("$Id: apt-probeset-genotype-stages.cpp,v 1.265 2009-10-14 17:46:11 awilli Exp $");
    const string versionToReport = version + " " + cvsId;
    const string execGuid = affxutil::Guid::GenerateNewGuid();
    
    /* Parse options. */
    engine.setUsage(
                    "apt-probeset-genotype-stages - test-program....\n"
                    );
    ...
    engine.openStandardLog("apt-probeset-genotype-stages.log",logOut,log);
