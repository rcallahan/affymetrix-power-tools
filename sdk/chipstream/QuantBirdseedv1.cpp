...
SelfDoc QuantBirdseedv1::explainSelf() {
    SelfDoc doc;
    setupSelfDoc(doc);
    return doc;
}

std::vector<SelfDoc::Opt> QuantBirdseedv1::getDefaultDocOptions() {
    ...
    birdseed::v1::getSelfDocOptions(&opts);
    return opts;
}

SelfCreate *QuantBirdseedv1::newObject(std::map<std::string,std::string> &param) {
    ...
    QuantBirdseedv1 *obj = new QuantBirdseedv1(confThreshold, correctionFactor);
...
// Index is birdseedv1::CallAndConfidence::Call
 ...
 */
void QuantBirdseedv1::computeEstimate()
{
    ...
    birdseed::v1::GenderAwareGenotypeCaller caller(intensities,
                                               m_Genders,
                                               m_PriorsReader.get(),
    ...
    for (size_t i = 0; i < intensities.numRows(); ++i) {
        birdseed::v1::CallAndConfidence callAndConfidence = caller.nextCall();
...
void QuantBirdseedv1::setPriorFile(const std::string& priorsPath, const string &chipType,
