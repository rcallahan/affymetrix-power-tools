...
int main(int argc,const char* argv[]) {
  ...
  try {
    ...
    ExportImputeFormatEngine engine;
    ...
    /* Parse options. */
    engine.setUsage("apt-table-to-impute - A program to convert the apt-probeset-genotype\n"
                    "table output (calls and confidences txt files) to an input format\n"
                    "impute will accept."
                    "usage:\n"
                    "   apt-table-to-impute ... \\\n"
                    "        ...");
    ...
    engine.openStandardLog("apt-table-to-impute.log",logOut,log);
