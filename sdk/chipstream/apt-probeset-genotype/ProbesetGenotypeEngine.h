...
class ProbesetGenotypeEngine : public BaseEngine {

    public:

        virtual std::string getEngineName() { return ProbesetGenotypeEngine::EngineName(); }
        static const std::string EngineName() { return "ProbesetGenotypeEngine"; }
        /**
         * Constructor
         */
        ...
        class Reg : public EngineReg {
            public:
                ...
                Reg() : EngineReg(ProbesetGenotypeEngine::EngineName()) { }
        
                /*! Creates an object.
                 * @return The object.
                 */
                BaseEngine *MakeObject() { return new ProbesetGenotypeEngine; }
        };
         ...
         */
        static ProbesetGenotypeEngine * FromBase(BaseEngine *engine);
...
/* The following method is part of Chuck's spf filter code that creates a
 * filtered spf based on the probeset subset list.  It is de-activated for now
 * until we can find a way to implement in such a way that normalization still
 * happens for all pm intensities, not just the subsetted list */
 ...
 * happens for all pm intensities, not just the subsetted list */
        ...
        void writeFilteredSpf(const std::string &filteredSpf, 
                              ChipLayout &m_ChipLayout, 
                              const std::vector<const char *> &toRunProbesets);
...
//                   const std::string &spfFiltFile="");
        ...
        void closeGlobalA5();
        int getLayoutVersion(ChipLayout &layout);
    ...
    private:
        ...
        std::string m_ChipViewMd5Sum;
        /// Md5 of entire file given with the "--probeset-ids" option
        std::string m_probeset_ids_md5sum;

  /// the vector of probeset names to process with snpqcChipSummary
  std::vector<std::string> m_snpqc_probesets;
};
